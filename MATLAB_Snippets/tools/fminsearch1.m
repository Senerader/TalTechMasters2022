...
%FMINSEARCH1 Multidimensional unconstrained nonlinear minimization (Nelder-Mead).
%   X = FMINSEARCH1(FUN,X0) starts at X0 and attempts to find a local minimizer 
...
%   or matrix.
%
%   X = FMINSEARCH1(FUN,X0,OPTIONS)  minimizes with the default optimization
...
%
%   X = FMINSEARCH1(PROBLEM) finds the minimum for PROBLEM. PROBLEM is a
%   structure with the function FUN in PROBLEM.objective, the start point
%   in PROBLEM.x0, the options structure in PROBLEM.options, and solver
%   name 'fminsearch' in PROBLEM.solver. The PROBLEM structure must have
%   all the fields.
%
%   [X,FVAL]= FMINSEARCH1(...) returns the value of the objective function,
%   described in FUN, at X.
%
%   [X,FVAL,EXITFLAG] = FMINSEARCH1(...) returns an EXITFLAG that describes 
...
%
%   [X,FVAL,EXITFLAG,OUTPUT] = FMINSEARCH1(...) returns a structure
...
%        c = 1.5;                        % The parameter.
%        X = fminsearch1(@(x) f(x,c),[0.3;1])
%        
%   FMINSEARCH1 uses the Nelder-Mead simplex (direct search) method.
%
...
%   Modified by AT for illustrative purposes
...
if haveoutputfcn || haveplotfcn
    [xOutputfcn, optimValues, stop] = callOutputAndPlotFcns(outputfcn,plotfcns,v(:,1),xOutputfcn,'init',itercount, ...
        func_evals, how, fv(:,1),v,varargin{:});
    if stop
...
% OutputFcn and PlotFcns call
if haveoutputfcn || haveplotfcn
    callOutputAndPlotFcns(outputfcn,plotfcns,x,xOutputfcn,'done',itercount, func_evals, how, fval, v, varargin{:});
end
